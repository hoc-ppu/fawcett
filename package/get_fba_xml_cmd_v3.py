...
def process_xml(input_xml, input_date):
    ...
    input_date_object = date(int(input_date.split(
        '-')[0]), int(input_date.split('-')[1]), int(input_date.split('-')[2]))
    ...
    if len(day_elements) < 1:
        input('Error:\tCan\'t seem to find the date you are after. '
    ...
    for day_element in day_elements:
        ...
        for section in sections:
            sections_in_day_list.append(section.findtext(
        ...
        for section in sections:
            ...
            for dayItem in dayItems:
                ...
                child_day_items = dayItem.find(
                    'BusinessItemDetail/ChildDayItems')
                ...
                if day_item_type.text == 'SectionDayDivider' and dayItem.find('Title') is not None:
                    last_gray_heading_text = dayItem.findtext(
                        'Title', default='').upper()
                    if last_gray_heading_text.upper() not in ('BUSINESS OF THE DAY',
                        ...
                        if last_gray_heading_text.upper() in ('QUESTIONS', 'ADJOURNMENT DEBATE'):
                            ...
                            if next_day_item is not None and next_day_item.findtext('DayItemType', default='') != 'SectionDayDivider':
                                SubElement(
                                    output_root, 'BusinessItemHeading').text = title
                ...
                if business_item_type is not None:
                    ...
                    if business_item_type.text == 'Substantive Question':
                        ...
                        if time_ele is not None:
                            formatted_time = op_functions.format_time(
                                time_ele.text)
                        SubElement(
                            output_root, 'QuestionTimeing').text = f'{formatted_time}\t{title}'
                    ...
                    if business_item_type.text == 'Adjournment Debate':
                        ...
                        # westminster hall
                            ...
                            adjourn_ele.text = op_functions.format_time(dayItem.findtext(
                                'BusinessItemDetail/Time', default='')) + '\t' + title_no_end_punctuation + ': '
                        else:
                            continue
                    ...
                    if business_item_type.text not in ('Adjournment Debate', 'Petition'):
                ...
                op_functions.notes_relevant_docs(dayItem, output_root,
                                                 has_children, day_item_is_child)

    # get the path to input file
    pwd = path.dirname(path.abspath(input_xml))
    # write out the file
    filename = path.basename(input_xml).replace(
        'as-downloaded-', '').split('.')[0]
