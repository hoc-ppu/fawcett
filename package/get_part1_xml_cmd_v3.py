...
def process_xml(input_xml, input_date):
    ...
    for section in sections:
        ...
        if section_name == 'Chamber':
            ...
            SubElement(
                output_root, 'OPHeading1').text = 'Business Today: Chamber'
            ...
            if dayItem_prayers is not None and dayItem_prayers.find('Title').text.upper().strip() == 'PRAYERS':
                prayer_time = dayItem_prayers.findtext(
                    'BusinessItemDetail/Time')
            ...
            SubElement(
                output_root, 'OPHeading1').text = 'Business Today: Westminster Hall'
        ...
        for dayItem in dayItem_all:
            ...
            if day_item_type.text == 'SectionDayDivider':
                ...
                SubElement(
                    output_root, 'OPHeading2').text = day_item_title_text
                # also add any notes
                ...
                if title is not None and day_item_title_text.upper() == 'URGENT QUESTIONS AND STATEMENTS':
                    time_in_op_form = op_functions.format_time(
                        dayItem.getnext().findtext('BusinessItemDetail/Time', default=''))
    ...
    try:
        ...
        try:
            ...
            print('Error:\tThe file ' + path.abspath(filepath + '2' +
                  fileextension) + ' dose not seem to be writable.')
...
def sort_and_append_written_statemetns(day_items, output_root):
    ...
    for day_item in day_items[1:]:
        ...
        answering_body_name = day_item.findtext(
            'BusinessItemDetail/AnsweringBodyName')
        if answering_body_name:
            ...
            if answering_body_name not in leave_alone:
                ...
                answering_body_name = answering_bodies_lookup.get(
